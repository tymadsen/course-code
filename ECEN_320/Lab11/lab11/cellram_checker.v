...
`include "cellram_parameters.vh"
	...
	// MJW
        $display("*****  Notice! *****");
        $display("** This memory model has been modified for the ECEN 320 frame buffer laboratory. ");
        $display("** The memory model does not behave as described in the device data sheet. ");
        $display("** The memory model will not simulate writes and will provide the following data on a read:"); 
        $display("**  - the model will output 'X' values when invalid image locations are read");
        $display("**  - For a valid image location, a memory read will return:");
        $display("**    data[15:12] - addr[21:18]");
        $display("**    data[11:8] - addr[2:0] & '1'");
        $display("**    data[7:4] - addr[12:9]");
        $display("**    data[3:0] - addr[2:0] & '0'");
   end
    ...
    function [DQ_BITS-1:0] memory_read;
		...
		//if (memory_addr_exists(addr)) begin
                ...
                //    memory_read = {DQ_BITS{1'bx}};
                //end
		// MJW: Override default function with a read that assumes
		// valid pixel data in the valid image regions and
		// invalid pixel data out of the image region.
		if (addr[8:0] >= 320 | addr[17:9] >= 520) begin
		     // invalid region of image memory
		     memory_read = {DQ_BITS{1'bx}};
                end else begin
		     //memory_read = {addr[20:18],addr[13:9],addr[7:0]};
		     memory_read = {addr[21:18],
      				    addr[2:0],1'b1,
                                    addr[12:9],
                                    addr[2:0],1'b0};
                end
		...
		//
		// memory value:
                //  15:13 - Lower three bits of image number
		//          addr[20:18]
                //  12:8  - pixel_y(4 downto 0)
		//          addr[13:9]
                //  7:0  - pixel_x(8 downto 1)
		//          addr[7:0]
		//if (addr[3] ^ addr[11]) begin
		//   memory_read = 16'b0001000100100010;
		//end else begin
		//   memory_read = 16'b0011001101000100;
		//end
            end
        end
    endfunction
